    ...
    */
    function sites_web_domain_edit($event_name, $page_form) {
        ...
        if($_SESSION["s"]["user"]["typ"] != 'admin' && !$app->auth->has_clients($_SESSION['s']['user']['userid'])) {                    
            ...
            $client = $app->db->queryOneRecord("SELECT client_id, name FROM sys_group WHERE sys_group.groupid = $client_group_id");
            $client_id = intval($client["client_id"]);
	    $username = $client["name"];
        } else {                
            //$client_id = intval($this->dataRecord["client_group_id"]);
            $client = $app->db->queryOneRecord("SELECT client_id, name FROM sys_group WHERE sys_group.groupid = ".intval($page_form->dataRecord["client_group_id"]));
            $client_id = intval($client["client_id"]);
	    $username = $client["name"];
	 ...
	 $system_user = $app->db->quote($username.$page_form->id);            //we change user
         $system_group = $app->db->quote($username);                     //we change group
	    $document_root = str_replace("[username]",$username,$document_root);
                $document_root = str_replace("[website_domain]",$page_form->dataRecord['domain'],$document_root);
                //My FUCKING PRAVKAS END
